...
mod tests {
    ...
    use crate::session::session_state::{SessionState, SessionStateEnum};
    ...
    impl Application for MockApp {
        fn on_create(&self, session_id: &crate::session::session_id::SessionID) {}

        fn on_logon(&self, session_id: &crate::session::session_id::SessionID) {
            todo!()
        }

        fn on_logout(&self, session_id: &crate::session::session_id::SessionID) {
            todo!()
        }

        fn to_admin(&self, msg: &'_ Message, session_id: &crate::session::session_id::SessionID) {
            todo!()
        }
        ...
        fn to_app(
            &self,
            msg: &'_ Message,
            session_id: &crate::session::session_id::SessionID,
        ...
        fn from_admin(
            &self,
            msg: &'_ Message,
            session_id: &'_ crate::session::session_id::SessionID,
        ) -> crate::errors::MessageRejectErrorResult {
            todo!()
        }
    ...
    impl SessionSuiteRig {
        ...
        fn state(&self, state: SessionStateEnum) -> bool {
            if self.session.is_none() {
                return false;
            }
            if self.session.as_ref().unwrap().sm.state.is_none() {
                return false;
            }

            // matches!(self.session.as_ref().unwrap().sm.state.as_ref().unwrap())
