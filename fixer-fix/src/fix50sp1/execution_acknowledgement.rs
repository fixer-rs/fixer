...
/// `ExecutionAcknowledgement` is the `fix50sp1` `ExecutionAcknowledgement` type, `MsgType` = BN.
pub struct ExecutionAcknowledgement {
    pub message: Message,
}

impl ExecutionAcknowledgement {
    /// Creates a new `ExecutionAcknowledgement` with required fields.
    pub fn new(order_id: field::OrderIDField, exec_ack_status: field::ExecAckStatusField, exec_id: field::ExecIDField, side: field::SideField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("BN".to_string()));

        msg.body.set_field(tag::ORDER_ID, order_id.0);

        msg.body.set_field(tag::EXEC_ACK_STATUS, exec_ack_status.0);
    ...
    /// Creates a `ExecutionAcknowledgement` from an existing `Message`.
    ...
    /// Sets `ExecAckStatus`, Tag 1036.
    pub fn set_exec_ack_status(&mut self, v: String) {
        self.message.body.set_field(tag::EXEC_ACK_STATUS, FIXString::from(v));
    }

    /// Gets `ExecAckStatus`, Tag 1036.
    pub fn get_exec_ack_status(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::ExecAckStatusField::new(String::new());
        self.message.body.get_field(tag::EXEC_ACK_STATUS, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `ExecAckStatus` is present, Tag 1036.
    pub fn has_exec_ack_status(&self) -> bool {
        self.message.body.has(tag::EXEC_ACK_STATUS)
    }
...
/// `RouteOut` is the callback type for routing `ExecutionAcknowledgement` messages.
pub type RouteOut = fn(msg: ExecutionAcknowledgement, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `ExecutionAcknowledgement`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(ExecutionAcknowledgement::from_message(msg.clone()), session_id)
    };
    ("8", "BN", Box::new(r))
}
