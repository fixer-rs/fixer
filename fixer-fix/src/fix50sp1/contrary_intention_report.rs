...
/// `ContraryIntentionReport` is the `fix50sp1` `ContraryIntentionReport` type, `MsgType` = BO.
pub struct ContraryIntentionReport {
    pub message: Message,
}

impl ContraryIntentionReport {
    /// Creates a new `ContraryIntentionReport` with required fields.
    pub fn new(cont_int_rpt_id: field::ContIntRptIDField, clearing_business_date: field::ClearingBusinessDateField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("BO".to_string()));

        msg.body.set_field(tag::CONT_INT_RPT_ID, cont_int_rpt_id.0);
    ...
    /// Creates a `ContraryIntentionReport` from an existing `Message`.
    ...
    /// Sets `ContIntRptID`, Tag 977.
    pub fn set_cont_int_rpt_id(&mut self, v: String) {
        self.message.body.set_field(tag::CONT_INT_RPT_ID, FIXString::from(v));
    }

    /// Gets `ContIntRptID`, Tag 977.
    pub fn get_cont_int_rpt_id(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::ContIntRptIDField::new(String::new());
        self.message.body.get_field(tag::CONT_INT_RPT_ID, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `ContIntRptID` is present, Tag 977.
    pub fn has_cont_int_rpt_id(&self) -> bool {
        self.message.body.has(tag::CONT_INT_RPT_ID)
    }
    ...
    /// Sets `InputSource`, Tag 979.
    pub fn set_input_source(&mut self, v: String) {
        self.message.body.set_field(tag::INPUT_SOURCE, FIXString::from(v));
    }

    /// Gets `InputSource`, Tag 979.
    pub fn get_input_source(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::InputSourceField::new(String::new());
        self.message.body.get_field(tag::INPUT_SOURCE, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `InputSource` is present, Tag 979.
    pub fn has_input_source(&self) -> bool {
        self.message.body.has(tag::INPUT_SOURCE)
    }
    ...
    /// Sets `LateIndicator`, Tag 978.
    pub fn set_late_indicator(&mut self, v: bool) {
        self.message.body.set_field(tag::LATE_INDICATOR, fixer::fix_boolean::FIXBoolean::from(v));
    }

    /// Gets `LateIndicator`, Tag 978.
    pub fn get_late_indicator(&self) -> Result<bool, MessageRejectErrorEnum> {
        let mut fld = field::LateIndicatorField::new(false);
        self.message.body.get_field(tag::LATE_INDICATOR, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `LateIndicator` is present, Tag 978.
    pub fn has_late_indicator(&self) -> bool {
        self.message.body.has(tag::LATE_INDICATOR)
    }
    ...
    /// Sets `NoExpiration`, Tag 981.
    pub fn set_no_expiration(&mut self, f: NoExpirationRepeatingGroup) {
        self.message.body.set_group(f.0);
    }

    /// Gets `NoExpiration`, Tag 981.
    pub fn get_no_expiration(&self) -> Result<NoExpirationRepeatingGroup, MessageRejectErrorEnum> {
        let g = NoExpirationRepeatingGroup::new();
        Ok(NoExpirationRepeatingGroup(self.message.body.get_group(g.0)?))
    }


    /// Returns true if `NoExpiration` is present, Tag 981.
    pub fn has_no_expiration(&self) -> bool {
        self.message.body.has(tag::NO_EXPIRATION)
    }
...
/// `RouteOut` is the callback type for routing `ContraryIntentionReport` messages.
pub type RouteOut = fn(msg: ContraryIntentionReport, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `ContraryIntentionReport`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(ContraryIntentionReport::from_message(msg.clone()), session_id)
    };
    ("8", "BO", Box::new(r))
}
...
/// `NoExpiration` is an entry in the `NoExpiration` repeating group, Tag 981.
pub struct NoExpiration<G>(pub G);

impl<G: Borrow<Group>> NoExpiration<G> {
    fn group(&self) -> &Group {
        self.0.borrow()
    }
    ...
    /// Gets `ExpirationQtyType`, Tag 982.
    pub fn get_expiration_qty_type(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::ExpirationQtyTypeField::new(0);
        self.group().field_map.get_field(tag::EXPIRATION_QTY_TYPE, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `ExpirationQtyType` is present, Tag 982.
    pub fn has_expiration_qty_type(&self) -> bool {
        self.group().field_map.has(tag::EXPIRATION_QTY_TYPE)
    }
    ...
    /// Gets `ExpQty`, Tag 983.
    pub fn get_exp_qty(&self) -> Result<Decimal, MessageRejectErrorEnum> {
        let mut fld = field::ExpQtyField::new(Decimal::ZERO, 0);
        self.group().field_map.get_field(tag::EXP_QTY, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `ExpQty` is present, Tag 983.
    pub fn has_exp_qty(&self) -> bool {
        self.group().field_map.has(tag::EXP_QTY)
    }


}

impl<G: BorrowMut<Group>> NoExpiration<G> {
    fn group_mut(&mut self) -> &mut Group {
        self.0.borrow_mut()
    }
    ...
    /// Sets `ExpirationQtyType`, Tag 982.
    pub fn set_expiration_qty_type(&mut self, v: isize) {
        self.group_mut().field_map.set_field(tag::EXPIRATION_QTY_TYPE, fixer::fix_int::FIXInt::from(v));
    }
    ...
    /// Sets `ExpQty`, Tag 983.
    pub fn set_exp_qty(&mut self, val: Decimal, scale: i32) {
        self.group_mut().field_map.set_field(tag::EXP_QTY, fixer::fix_decimal::FIXDecimal { decimal: val, scale });
    }
...
/// `NoExpirationRepeatingGroup` is the `NoExpiration` repeating group, Tag 981.
pub struct NoExpirationRepeatingGroup(pub RepeatingGroup);

impl NoExpirationRepeatingGroup {
    /// Creates an empty `NoExpiration` group with the template from the FIX spec.
    pub fn new() -> Self {
        let template: GroupTemplate = vec![


            group_element(tag::EXPIRATION_QTY_TYPE),



            group_element(tag::EXP_QTY),


        ];
        Self(RepeatingGroup::new(tag::NO_EXPIRATION, template))
    }

    /// Appends an entry and returns it for population.
    pub fn add(&mut self) -> NoExpiration<&mut Group> {
        NoExpiration(self.0.add())
    }

    /// Returns the `i`th entry.
    pub fn get(&self, i: usize) -> NoExpiration<&Group> {
        NoExpiration(self.0.get(i))
    }
...
impl Default for NoExpirationRepeatingGroup {
    fn default() -> Self {
        Self::new()
    }
}
