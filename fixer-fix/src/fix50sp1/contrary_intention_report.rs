...
impl ContraryIntentionReport {
    ...
    pub fn set_no_expiration(&mut self, v: isize) {
        self.message.body.set_field(tag::NO_EXPIRATION, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `NoExpiration`, Tag 981.
    pub fn get_no_expiration(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::NoExpirationField::new(0);
        self.message.body.get_field(tag::NO_EXPIRATION, &mut fld.0)?;
        Ok(fld.value())
    }
