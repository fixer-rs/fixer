...
/// `PositionMaintenanceRequest` is the `fix50sp1` `PositionMaintenanceRequest` type, `MsgType` = AL.
pub struct PositionMaintenanceRequest {
    pub message: Message,
}

impl PositionMaintenanceRequest {
    /// Creates a new `PositionMaintenanceRequest` with required fields.
    pub fn new(pos_trans_type: field::PosTransTypeField, pos_maint_action: field::PosMaintActionField, clearing_business_date: field::ClearingBusinessDateField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AL".to_string()));
    ...
    /// Creates a `PositionMaintenanceRequest` from an existing `Message`.
...
/// `RouteOut` is the callback type for routing `PositionMaintenanceRequest` messages.
pub type RouteOut = fn(msg: PositionMaintenanceRequest, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `PositionMaintenanceRequest`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(PositionMaintenanceRequest::from_message(msg.clone()), session_id)
    };
    ("8", "AL", Box::new(r))
}
