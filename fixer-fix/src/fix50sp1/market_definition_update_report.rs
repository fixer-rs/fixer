...
impl MarketDefinitionUpdateReport {
    /// Creates a new `MarketDefinitionUpdateReport` with required fields.
    pub fn new(market_report_id: field::MarketReportIDField, market_id: field::MarketIDField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("BV".to_string()));
    ...
    /// Creates a `MarketDefinitionUpdateReport` from an existing `Message`.
    ...
    /// Sets `MarketUpdateAction`, Tag 1395.
    pub fn set_market_update_action(&mut self, v: String) {
        self.message.body.set_field(tag::MARKET_UPDATE_ACTION, FIXString::from(v));
    }

    /// Gets `MarketUpdateAction`, Tag 1395.
    pub fn get_market_update_action(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::MarketUpdateActionField::new(String::new());
        self.message.body.get_field(tag::MARKET_UPDATE_ACTION, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `MarketUpdateAction` is present, Tag 1395.
    pub fn has_market_update_action(&self) -> bool {
        self.message.body.has(tag::MARKET_UPDATE_ACTION)
    }
...
/// `RouteOut` is the callback type for routing `MarketDefinitionUpdateReport` messages.
pub type RouteOut = fn(msg: MarketDefinitionUpdateReport, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `MarketDefinitionUpdateReport`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(MarketDefinitionUpdateReport::from_message(msg.clone()), session_id)
    };
    ("8", "BV", Box::new(r))
}
