...
/// `TradeCaptureReport` is the `fix50sp1` `TradeCaptureReport` type, `MsgType` = AE.
...
impl<G: Borrow<Group>> NoSides<G> {
    ...
    /// Gets `SideQty`, Tag 1009.
    pub fn get_side_qty(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::SideQtyField::new(0);
        self.group().field_map.get_field(tag::SIDE_QTY, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `SideQty` is present, Tag 1009.
    pub fn has_side_qty(&self) -> bool {
        self.group().field_map.has(tag::SIDE_QTY)
    }
    ...
    pub fn get_exec_ref_id(&self) -> Result<String, MessageRejectErrorEnum> {
        ...
        self.group().field_map.get_field(tag::EXEC_REF_ID, &mut fld.0)?;
        Ok(fld.value().to_string())
    }
    ...
    pub fn has_exec_ref_id(&self) -> bool {
        self.group().field_map.has(tag::EXEC_REF_ID)
    }
...
impl<G: BorrowMut<Group>> NoSides<G> {
    ...
    /// Sets `SideQty`, Tag 1009.
    pub fn set_side_qty(&mut self, v: isize) {
        self.group_mut().field_map.set_field(tag::SIDE_QTY, fixer::fix_int::FIXInt::from(v));
    }
    ...
    pub fn set_exec_ref_id(&mut self, v: String) {
        self.group_mut().field_map.set_field(tag::EXEC_REF_ID, FIXString::from(v));
    }
