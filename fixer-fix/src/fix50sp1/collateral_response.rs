...
/// `CollateralResponse` is the `fix50sp1` `CollateralResponse` type, `MsgType` = AZ.
pub struct CollateralResponse {
    pub message: Message,
}

impl CollateralResponse {
    /// Creates a new `CollateralResponse` with required fields.
    pub fn new(coll_resp_id: field::CollRespIDField, coll_asgn_resp_type: field::CollAsgnRespTypeField, transact_time: field::TransactTimeField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AZ".to_string()));

        msg.body.set_field(tag::COLL_RESP_ID, coll_resp_id.0);

        msg.body.set_field(tag::COLL_ASGN_RESP_TYPE, coll_asgn_resp_type.0);
    ...
    /// Creates a `CollateralResponse` from an existing `Message`.
    ...
    /// Sets `CollAsgnRejectReason`, Tag 906.
    pub fn set_coll_asgn_reject_reason(&mut self, v: isize) {
        self.message.body.set_field(tag::COLL_ASGN_REJECT_REASON, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `CollAsgnRejectReason`, Tag 906.
    pub fn get_coll_asgn_reject_reason(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::CollAsgnRejectReasonField::new(0);
        self.message.body.get_field(tag::COLL_ASGN_REJECT_REASON, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `CollAsgnRejectReason` is present, Tag 906.
    pub fn has_coll_asgn_reject_reason(&self) -> bool {
        self.message.body.has(tag::COLL_ASGN_REJECT_REASON)
    }




    /// Sets `CollAsgnRespType`, Tag 905.
    pub fn set_coll_asgn_resp_type(&mut self, v: isize) {
        self.message.body.set_field(tag::COLL_ASGN_RESP_TYPE, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `CollAsgnRespType`, Tag 905.
    pub fn get_coll_asgn_resp_type(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::CollAsgnRespTypeField::new(0);
        self.message.body.get_field(tag::COLL_ASGN_RESP_TYPE, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `CollAsgnRespType` is present, Tag 905.
    pub fn has_coll_asgn_resp_type(&self) -> bool {
        self.message.body.has(tag::COLL_ASGN_RESP_TYPE)
    }
    ...
    /// Sets `CollRespID`, Tag 904.
    pub fn set_coll_resp_id(&mut self, v: String) {
        self.message.body.set_field(tag::COLL_RESP_ID, FIXString::from(v));
    }

    /// Gets `CollRespID`, Tag 904.
    pub fn get_coll_resp_id(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::CollRespIDField::new(String::new());
        self.message.body.get_field(tag::COLL_RESP_ID, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `CollRespID` is present, Tag 904.
    pub fn has_coll_resp_id(&self) -> bool {
        self.message.body.has(tag::COLL_RESP_ID)
    }
...
/// `RouteOut` is the callback type for routing `CollateralResponse` messages.
pub type RouteOut = fn(msg: CollateralResponse, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `CollateralResponse`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(CollateralResponse::from_message(msg.clone()), session_id)
    };
    ("8", "AZ", Box::new(r))
}
