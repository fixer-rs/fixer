...
/// `CollateralInquiry` is the `fix50sp1` `CollateralInquiry` type, `MsgType` = BB.
pub struct CollateralInquiry {
    pub message: Message,
}

impl CollateralInquiry {
    /// Creates a new `CollateralInquiry` with required fields.
    pub fn new() -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("BB".to_string()));

        Self { message: msg }
    }

    /// Creates a `CollateralInquiry` from an existing `Message`.
    ...
    pub fn set_no_coll_inquiry_qualifier(&mut self, f: NoCollInquiryQualifierRepeatingGroup) {
        self.message.body.set_group(f.0);
    }

    /// Gets `NoCollInquiryQualifier`, Tag 938.
    pub fn get_no_coll_inquiry_qualifier(&self) -> Result<NoCollInquiryQualifierRepeatingGroup, MessageRejectErrorEnum> {
        let g = NoCollInquiryQualifierRepeatingGroup::new();
        Ok(NoCollInquiryQualifierRepeatingGroup(self.message.body.get_group(g.0)?))
    }
...
/// `RouteOut` is the callback type for routing `CollateralInquiry` messages.
pub type RouteOut = fn(msg: CollateralInquiry, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `CollateralInquiry`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(CollateralInquiry::from_message(msg.clone()), session_id)
    };
    ("8", "BB", Box::new(r))
}


/// `NoCollInquiryQualifier` is an entry in the `NoCollInquiryQualifier` repeating group, Tag 938.
pub struct NoCollInquiryQualifier<G>(pub G);

impl<G: Borrow<Group>> NoCollInquiryQualifier<G> {
    fn group(&self) -> &Group {
        self.0.borrow()
    }
    ...
    /// Gets `CollInquiryQualifier`, Tag 896.
    pub fn get_coll_inquiry_qualifier(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::CollInquiryQualifierField::new(0);
        self.group().field_map.get_field(tag::COLL_INQUIRY_QUALIFIER, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `CollInquiryQualifier` is present, Tag 896.
    pub fn has_coll_inquiry_qualifier(&self) -> bool {
        self.group().field_map.has(tag::COLL_INQUIRY_QUALIFIER)
    }


}

impl<G: BorrowMut<Group>> NoCollInquiryQualifier<G> {
    ...
    /// Sets `CollInquiryQualifier`, Tag 896.
    pub fn set_coll_inquiry_qualifier(&mut self, v: isize) {
        self.group_mut().field_map.set_field(tag::COLL_INQUIRY_QUALIFIER, fixer::fix_int::FIXInt::from(v));
    }
...
/// `NoCollInquiryQualifierRepeatingGroup` is the `NoCollInquiryQualifier` repeating group, Tag 938.
pub struct NoCollInquiryQualifierRepeatingGroup(pub RepeatingGroup);

impl NoCollInquiryQualifierRepeatingGroup {
    /// Creates an empty `NoCollInquiryQualifier` group with the template from the FIX spec.
    pub fn new() -> Self {
        let template: GroupTemplate = vec![


            group_element(tag::COLL_INQUIRY_QUALIFIER),


        ];
        Self(RepeatingGroup::new(tag::NO_COLL_INQUIRY_QUALIFIER, template))
    }

    /// Appends an entry and returns it for population.
    pub fn add(&mut self) -> NoCollInquiryQualifier<&mut Group> {
        NoCollInquiryQualifier(self.0.add())
    }

    /// Returns the `i`th entry.
    pub fn get(&self, i: usize) -> NoCollInquiryQualifier<&Group> {
        NoCollInquiryQualifier(self.0.get(i))
    }
...
impl Default for NoCollInquiryQualifierRepeatingGroup {
    fn default() -> Self {
        Self::new()
    }
}
