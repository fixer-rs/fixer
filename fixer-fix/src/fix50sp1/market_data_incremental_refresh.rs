...
/// `MarketDataIncrementalRefresh` is the `fix50sp1` `MarketDataIncrementalRefresh` type, `MsgType` = X.
...
impl<G: Borrow<Group>> NoMDEntries<G> {
    ...
    pub fn get_halt_reason_char(&self) -> Result<String, MessageRejectErrorEnum> {
        ...
        self.group().field_map.get_field(tag::HALT_REASON_CHAR, &mut fld.0)?;
        Ok(fld.value().to_string())
    }
    ...
    pub fn has_halt_reason_char(&self) -> bool {
        self.group().field_map.has(tag::HALT_REASON_CHAR)
    }
...
impl<G: BorrowMut<Group>> NoMDEntries<G> {
    ...
    pub fn set_halt_reason_char(&mut self, v: String) {
        self.group_mut().field_map.set_field(tag::HALT_REASON_CHAR, FIXString::from(v));
    }
...
impl NoMDEntriesRepeatingGroup {
    ...
    pub fn new() -> Self {
        let template: GroupTemplate = vec![
            ...
            group_element(tag::HALT_REASON_CHAR),
