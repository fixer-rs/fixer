...
/// `SecurityDefinitionUpdateReport` is the `fix50sp1` `SecurityDefinitionUpdateReport` type, `MsgType` = BP.
pub struct SecurityDefinitionUpdateReport {
    pub message: Message,
}

impl SecurityDefinitionUpdateReport {
    /// Creates a new `SecurityDefinitionUpdateReport` with required fields.
    pub fn new() -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("BP".to_string()));
...
/// `RouteOut` is the callback type for routing `SecurityDefinitionUpdateReport` messages.
pub type RouteOut = fn(msg: SecurityDefinitionUpdateReport, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `SecurityDefinitionUpdateReport`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(SecurityDefinitionUpdateReport::from_message(msg.clone()), session_id)
    };
    ("8", "BP", Box::new(r))
}
