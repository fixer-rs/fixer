...
/// `ListStatus` is the `fix50sp1` `ListStatus` type, `MsgType` = N.
...
impl ListStatus {
    ...
    /// Sets `ListRejectReason`, Tag 1386.
    pub fn set_list_reject_reason(&mut self, v: isize) {
        self.message.body.set_field(tag::LIST_REJECT_REASON, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `ListRejectReason`, Tag 1386.
    pub fn get_list_reject_reason(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::ListRejectReasonField::new(0);
        self.message.body.get_field(tag::LIST_REJECT_REASON, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `ListRejectReason` is present, Tag 1386.
    pub fn has_list_reject_reason(&self) -> bool {
        self.message.body.has(tag::LIST_REJECT_REASON)
    }
