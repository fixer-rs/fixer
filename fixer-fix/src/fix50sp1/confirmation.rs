...
/// `Confirmation` is the `fix50sp1` `Confirmation` type, `MsgType` = AK.
...
impl Confirmation {
    ...
    pub fn new(confirm_id: field::ConfirmIDField, confirm_trans_type: field::ConfirmTransTypeField, confirm_type: field::ConfirmTypeField, confirm_status: field::ConfirmStatusField, transact_time: field::TransactTimeField, trade_date: field::TradeDateField, alloc_qty: field::AllocQtyField, side: field::SideField, alloc_account: field::AllocAccountField, avg_px: field::AvgPxField, gross_trade_amt: field::GrossTradeAmtField, net_money: field::NetMoneyField) -> Self {
    ...
    pub fn set_no_capacities(&mut self, f: NoCapacitiesRepeatingGroup) {
        self.message.body.set_group(f.0);
    }

    /// Gets `NoCapacities`, Tag 862.
    pub fn get_no_capacities(&self) -> Result<NoCapacitiesRepeatingGroup, MessageRejectErrorEnum> {
        let g = NoCapacitiesRepeatingGroup::new();
        Ok(NoCapacitiesRepeatingGroup(self.message.body.get_group(g.0)?))
    }
...
/// `NoCapacities` is an entry in the `NoCapacities` repeating group, Tag 862.
pub struct NoCapacities<G>(pub G);

impl<G: Borrow<Group>> NoCapacities<G> {
    fn group(&self) -> &Group {
        self.0.borrow()
    }
    ...
    /// Gets `OrderCapacityQty`, Tag 863.
    pub fn get_order_capacity_qty(&self) -> Result<Decimal, MessageRejectErrorEnum> {
        let mut fld = field::OrderCapacityQtyField::new(Decimal::ZERO, 0);
        self.group().field_map.get_field(tag::ORDER_CAPACITY_QTY, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `OrderCapacityQty` is present, Tag 863.
    pub fn has_order_capacity_qty(&self) -> bool {
        self.group().field_map.has(tag::ORDER_CAPACITY_QTY)
    }


}

impl<G: BorrowMut<Group>> NoCapacities<G> {
    ...
    /// Sets `OrderCapacityQty`, Tag 863.
    pub fn set_order_capacity_qty(&mut self, val: Decimal, scale: i32) {
        self.group_mut().field_map.set_field(tag::ORDER_CAPACITY_QTY, fixer::fix_decimal::FIXDecimal { decimal: val, scale });
    }
...
/// `NoCapacitiesRepeatingGroup` is the `NoCapacities` repeating group, Tag 862.
pub struct NoCapacitiesRepeatingGroup(pub RepeatingGroup);

impl NoCapacitiesRepeatingGroup {
    /// Creates an empty `NoCapacities` group with the template from the FIX spec.
    pub fn new() -> Self {
        let template: GroupTemplate = vec![
            ...
            group_element(tag::ORDER_CAPACITY_QTY),


        ];
        Self(RepeatingGroup::new(tag::NO_CAPACITIES, template))
    }

    /// Appends an entry and returns it for population.
    pub fn add(&mut self) -> NoCapacities<&mut Group> {
        NoCapacities(self.0.add())
    }

    /// Returns the `i`th entry.
    pub fn get(&self, i: usize) -> NoCapacities<&Group> {
        NoCapacities(self.0.get(i))
    }
...
impl Default for NoCapacitiesRepeatingGroup {
    fn default() -> Self {
        Self::new()
    }
}
