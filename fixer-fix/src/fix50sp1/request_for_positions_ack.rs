...
/// `RequestForPositionsAck` is the `fix50sp1` `RequestForPositionsAck` type, `MsgType` = AO.
pub struct RequestForPositionsAck {
    pub message: Message,
}

impl RequestForPositionsAck {
    /// Creates a new `RequestForPositionsAck` with required fields.
    pub fn new(pos_maint_rpt_id: field::PosMaintRptIDField, pos_req_result: field::PosReqResultField, pos_req_status: field::PosReqStatusField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AO".to_string()));

        msg.body.set_field(tag::POS_MAINT_RPT_ID, pos_maint_rpt_id.0);

        msg.body.set_field(tag::POS_REQ_RESULT, pos_req_result.0);

        msg.body.set_field(tag::POS_REQ_STATUS, pos_req_status.0);

        Self { message: msg }
    }

    /// Creates a `RequestForPositionsAck` from an existing `Message`.
    ...
    /// Sets `PosReqStatus`, Tag 729.
    pub fn set_pos_req_status(&mut self, v: isize) {
        self.message.body.set_field(tag::POS_REQ_STATUS, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `PosReqStatus`, Tag 729.
    pub fn get_pos_req_status(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::PosReqStatusField::new(0);
        self.message.body.get_field(tag::POS_REQ_STATUS, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `PosReqStatus` is present, Tag 729.
    pub fn has_pos_req_status(&self) -> bool {
        self.message.body.has(tag::POS_REQ_STATUS)
    }
...
/// `RouteOut` is the callback type for routing `RequestForPositionsAck` messages.
pub type RouteOut = fn(msg: RequestForPositionsAck, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `RequestForPositionsAck`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(RequestForPositionsAck::from_message(msg.clone()), session_id)
    };
    ("8", "AO", Box::new(r))
}
