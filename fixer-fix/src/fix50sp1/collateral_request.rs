...
/// `CollateralRequest` is the `fix50sp1` `CollateralRequest` type, `MsgType` = AX.
pub struct CollateralRequest {
    pub message: Message,
}

impl CollateralRequest {
    /// Creates a new `CollateralRequest` with required fields.
    pub fn new(coll_req_id: field::CollReqIDField, coll_asgn_reason: field::CollAsgnReasonField, transact_time: field::TransactTimeField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AX".to_string()));

        msg.body.set_field(tag::COLL_REQ_ID, coll_req_id.0);
    ...
    /// Creates a `CollateralRequest` from an existing `Message`.
...
/// `RouteOut` is the callback type for routing `CollateralRequest` messages.
pub type RouteOut = fn(msg: CollateralRequest, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `CollateralRequest`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(CollateralRequest::from_message(msg.clone()), session_id)
    };
    ("8", "AX", Box::new(r))
}
