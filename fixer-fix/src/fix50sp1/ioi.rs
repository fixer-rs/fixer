...
impl<G: Borrow<Group>> NoLegs<G> {
    ...
    /// Gets `LegIOIQty`, Tag 682.
    pub fn get_leg_ioi_qty(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::LegIOIQtyField::new(String::new());
        self.group().field_map.get_field(tag::LEG_IOI_QTY, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `LegIOIQty` is present, Tag 682.
    pub fn has_leg_ioi_qty(&self) -> bool {
        self.group().field_map.has(tag::LEG_IOI_QTY)
    }
...
impl<G: BorrowMut<Group>> NoLegs<G> {
    ...
    /// Sets `LegIOIQty`, Tag 682.
    pub fn set_leg_ioi_qty(&mut self, v: String) {
        self.group_mut().field_map.set_field(tag::LEG_IOI_QTY, FIXString::from(v));
    }
