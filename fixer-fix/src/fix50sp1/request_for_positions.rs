...
/// `RequestForPositions` is the `fix50sp1` `RequestForPositions` type, `MsgType` = AN.
pub struct RequestForPositions {
    pub message: Message,
}

impl RequestForPositions {
    /// Creates a new `RequestForPositions` with required fields.
    pub fn new(pos_req_id: field::PosReqIDField, pos_req_type: field::PosReqTypeField, clearing_business_date: field::ClearingBusinessDateField, transact_time: field::TransactTimeField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AN".to_string()));

        msg.body.set_field(tag::POS_REQ_ID, pos_req_id.0);

        msg.body.set_field(tag::POS_REQ_TYPE, pos_req_type.0);
    ...
    /// Creates a `RequestForPositions` from an existing `Message`.
...
/// `RouteOut` is the callback type for routing `RequestForPositions` messages.
pub type RouteOut = fn(msg: RequestForPositions, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `RequestForPositions`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(RequestForPositions::from_message(msg.clone()), session_id)
    };
    ("8", "AN", Box::new(r))
}
