...
/// `ContraryIntentionReport` is the `fix50` `ContraryIntentionReport` type, `MsgType` = BO.
...
impl<G: Borrow<Group>> NoExpiration<G> {
    ...
    /// Gets `ExpType`, Tag 982.
    pub fn get_exp_type(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::ExpTypeField::new(0);
        self.group().field_map.get_field(tag::EXP_TYPE, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `ExpType` is present, Tag 982.
    pub fn has_exp_type(&self) -> bool {
        self.group().field_map.has(tag::EXP_TYPE)
    }
...
impl<G: BorrowMut<Group>> NoExpiration<G> {
    ...
    /// Sets `ExpType`, Tag 982.
    pub fn set_exp_type(&mut self, v: isize) {
        self.group_mut().field_map.set_field(tag::EXP_TYPE, fixer::fix_int::FIXInt::from(v));
    }
