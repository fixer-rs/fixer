...
/// `SecurityDefinitionRequest` is the `fix50` `SecurityDefinitionRequest` type, `MsgType` = c.
...
impl SecurityDefinitionRequest {
    ...
    pub fn set_subscription_request_type(&mut self, f: SubscriptionRequestTypeRepeatingGroup) {
        self.message.body.set_group(f.0);
    }

    /// Gets `SubscriptionRequestType`, Tag 263.
    pub fn get_subscription_request_type(&self) -> Result<SubscriptionRequestTypeRepeatingGroup, MessageRejectErrorEnum> {
        let g = SubscriptionRequestTypeRepeatingGroup::new();
        Ok(SubscriptionRequestTypeRepeatingGroup(self.message.body.get_group(g.0)?))
    }
...
/// `SubscriptionRequestType` is an entry in the `SubscriptionRequestType` repeating group, Tag 263.
pub struct SubscriptionRequestType<G>(pub G);

impl<G: Borrow<Group>> SubscriptionRequestType<G> {
    fn group(&self) -> &Group {
        self.0.borrow()
    }
...
impl<G: BorrowMut<Group>> SubscriptionRequestType<G> {
    fn group_mut(&mut self) -> &mut Group {
        self.0.borrow_mut()
    }
...
/// `SubscriptionRequestTypeRepeatingGroup` is the `SubscriptionRequestType` repeating group, Tag 263.
pub struct SubscriptionRequestTypeRepeatingGroup(pub RepeatingGroup);

impl SubscriptionRequestTypeRepeatingGroup {
    /// Creates an empty `SubscriptionRequestType` group with the template from the FIX spec.
    pub fn new() -> Self {
        let template: GroupTemplate = vec![
        ...
        Self(RepeatingGroup::new(tag::SUBSCRIPTION_REQUEST_TYPE, template))
    }

    /// Appends an entry and returns it for population.
    pub fn add(&mut self) -> SubscriptionRequestType<&mut Group> {
        SubscriptionRequestType(self.0.add())
    }

    /// Returns the `i`th entry.
    pub fn get(&self, i: usize) -> SubscriptionRequestType<&Group> {
        SubscriptionRequestType(self.0.get(i))
    }
...
impl Default for SubscriptionRequestTypeRepeatingGroup {
    fn default() -> Self {
        Self::new()
    }
}
