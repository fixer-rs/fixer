...
/// `SecurityDefinition` is the `fix50` `SecurityDefinition` type, `MsgType` = d.
...
impl SecurityDefinition {
    ...
    pub fn set_clearing_business_date(&mut self, f: ClearingBusinessDateRepeatingGroup) {
        self.message.body.set_group(f.0);
    }

    /// Gets `ClearingBusinessDate`, Tag 715.
    pub fn get_clearing_business_date(&self) -> Result<ClearingBusinessDateRepeatingGroup, MessageRejectErrorEnum> {
        let g = ClearingBusinessDateRepeatingGroup::new();
        Ok(ClearingBusinessDateRepeatingGroup(self.message.body.get_group(g.0)?))
    }
...
/// `ClearingBusinessDate` is an entry in the `ClearingBusinessDate` repeating group, Tag 715.
pub struct ClearingBusinessDate<G>(pub G);

impl<G: Borrow<Group>> ClearingBusinessDate<G> {
    fn group(&self) -> &Group {
        self.0.borrow()
    }
...
impl<G: BorrowMut<Group>> ClearingBusinessDate<G> {
...
/// `ClearingBusinessDateRepeatingGroup` is the `ClearingBusinessDate` repeating group, Tag 715.
pub struct ClearingBusinessDateRepeatingGroup(pub RepeatingGroup);

impl ClearingBusinessDateRepeatingGroup {
    /// Creates an empty `ClearingBusinessDate` group with the template from the FIX spec.
    pub fn new() -> Self {
        ...
        Self(RepeatingGroup::new(tag::CLEARING_BUSINESS_DATE, template))
    }

    /// Appends an entry and returns it for population.
    pub fn add(&mut self) -> ClearingBusinessDate<&mut Group> {
        ClearingBusinessDate(self.0.add())
    }

    /// Returns the `i`th entry.
    pub fn get(&self, i: usize) -> ClearingBusinessDate<&Group> {
        ClearingBusinessDate(self.0.get(i))
    }
...
impl Default for ClearingBusinessDateRepeatingGroup {
    fn default() -> Self {
        Self::new()
    }
}
