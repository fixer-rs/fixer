...
impl Header<'_> {
    ...
    pub fn new(header: &mut FieldMap) -> Header<'_> {
        header.set_field(tag::BEGIN_STRING, FIXString::from("FIXT.1.1".to_string()));
        Header { header }
    }
    ...
    /// Sets `ApplVerID`, Tag 1128.
    pub fn set_appl_ver_id(&mut self, v: String) {
        self.header.set_field(tag::APPL_VER_ID, FIXString::from(v));
    }


    /// Returns true if `ApplVerID` is present, Tag 1128.
    pub fn has_appl_ver_id(&self) -> bool {
        self.header.has(tag::APPL_VER_ID)
    }
    ...
    /// Sets `CstmApplVerID`, Tag 1129.
    pub fn set_cstm_appl_ver_id(&mut self, v: String) {
        self.header.set_field(tag::CSTM_APPL_VER_ID, FIXString::from(v));
    }


    /// Returns true if `CstmApplVerID` is present, Tag 1129.
    pub fn has_cstm_appl_ver_id(&self) -> bool {
        self.header.has(tag::CSTM_APPL_VER_ID)
    }
