...
impl Reject {
    /// Creates a new `Reject` with required fields.
    pub fn new(ref_seq_num: field::RefSeqNumField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("3".to_string()));

        msg.body.set_field(tag::REF_SEQ_NUM, ref_seq_num.0);

        Self { message: msg }
    }

    /// Creates a `Reject` from an existing `Message`.
    ...
    /// Sets `RefTagID`, Tag 371.
    pub fn set_ref_tag_id(&mut self, v: isize) {
        self.message.body.set_field(tag::REF_TAG_ID, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `RefTagID`, Tag 371.
    pub fn get_ref_tag_id(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::RefTagIDField::new(0);
        self.message.body.get_field(tag::REF_TAG_ID, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `RefTagID` is present, Tag 371.
    pub fn has_ref_tag_id(&self) -> bool {
        self.message.body.has(tag::REF_TAG_ID)
    }




    /// Sets `SessionRejectReason`, Tag 373.
    pub fn set_session_reject_reason(&mut self, v: isize) {
        self.message.body.set_field(tag::SESSION_REJECT_REASON, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `SessionRejectReason`, Tag 373.
    pub fn get_session_reject_reason(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::SessionRejectReasonField::new(0);
        self.message.body.get_field(tag::SESSION_REJECT_REASON, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `SessionRejectReason` is present, Tag 373.
    pub fn has_session_reject_reason(&self) -> bool {
        self.message.body.has(tag::SESSION_REJECT_REASON)
    }
...
/// Returns the begin string, message type, and route function for `Reject`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(Reject::from_message(msg.clone()), session_id)
    };
    ("FIXT.1.1", "3", Box::new(r))
}
