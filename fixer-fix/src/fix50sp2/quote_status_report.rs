...
impl QuoteStatusReport {
    /// Creates a new `QuoteStatusReport` with required fields.
    pub fn new() -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AI".to_string()));

        Self { message: msg }
    }

    /// Creates a `QuoteStatusReport` from an existing `Message`.
...
/// `RouteOut` is the callback type for routing `QuoteStatusReport` messages.
pub type RouteOut = fn(msg: QuoteStatusReport, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `QuoteStatusReport`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(QuoteStatusReport::from_message(msg.clone()), session_id)
    };
    ("9", "AI", Box::new(r))
}
