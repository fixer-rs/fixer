...
impl OrderMassActionRequest {
    /// Creates a new `OrderMassActionRequest` with required fields.
    pub fn new(cl_ord_id: field::ClOrdIDField, mass_action_type: field::MassActionTypeField, mass_action_scope: field::MassActionScopeField, transact_time: field::TransactTimeField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("CA".to_string()));
    ...
    /// Creates a `OrderMassActionRequest` from an existing `Message`.
...
/// `RouteOut` is the callback type for routing `OrderMassActionRequest` messages.
pub type RouteOut = fn(msg: OrderMassActionRequest, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `OrderMassActionRequest`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(OrderMassActionRequest::from_message(msg.clone()), session_id)
    };
    ("9", "CA", Box::new(r))
}
