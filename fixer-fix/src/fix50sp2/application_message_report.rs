...
impl ApplicationMessageReport {
    ...
    pub fn new(appl_report_id: field::ApplReportIDField, appl_report_type: field::ApplReportTypeField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("BY".to_string()));

        msg.body.set_field(tag::APPL_REPORT_ID, appl_report_id.0);

        msg.body.set_field(tag::APPL_REPORT_TYPE, appl_report_type.0);
    ...
    pub fn set_appl_report_id(&mut self, v: String) {
        self.message.body.set_field(tag::APPL_REPORT_ID, FIXString::from(v));
    }

    /// Gets `ApplReportID`, Tag 1356.
    pub fn get_appl_report_id(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::ApplReportIDField::new(String::new());
        self.message.body.get_field(tag::APPL_REPORT_ID, &mut fld.0)?;
    ...
    pub fn has_appl_report_id(&self) -> bool {
        self.message.body.has(tag::APPL_REPORT_ID)
    }
    ...
    pub fn set_appl_report_type(&mut self, v: isize) {
        self.message.body.set_field(tag::APPL_REPORT_TYPE, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `ApplReportType`, Tag 1426.
    pub fn get_appl_report_type(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::ApplReportTypeField::new(0);
        self.message.body.get_field(tag::APPL_REPORT_TYPE, &mut fld.0)?;
        Ok(fld.value())
    }
    ...
    pub fn has_appl_report_type(&self) -> bool {
        self.message.body.has(tag::APPL_REPORT_TYPE)
    }
...
/// `RouteOut` is the callback type for routing `ApplicationMessageReport` messages.
pub type RouteOut = fn(msg: ApplicationMessageReport, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `ApplicationMessageReport`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(ApplicationMessageReport::from_message(msg.clone()), session_id)
    };
    ("9", "BY", Box::new(r))
}
...
impl<G: Borrow<Group>> NoApplIDs<G> {
    ...
    /// Gets `ApplNewSeqNum`, Tag 1399.
    pub fn get_appl_new_seq_num(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::ApplNewSeqNumField::new(0);
        self.group().field_map.get_field(tag::APPL_NEW_SEQ_NUM, &mut fld.0)?;
        Ok(fld.value())
    }


    /// Returns true if `ApplNewSeqNum` is present, Tag 1399.
    pub fn has_appl_new_seq_num(&self) -> bool {
        self.group().field_map.has(tag::APPL_NEW_SEQ_NUM)
    }
...
impl<G: BorrowMut<Group>> NoApplIDs<G> {
    ...
    /// Sets `ApplNewSeqNum`, Tag 1399.
    pub fn set_appl_new_seq_num(&mut self, v: isize) {
        self.group_mut().field_map.set_field(tag::APPL_NEW_SEQ_NUM, fixer::fix_int::FIXInt::from(v));
    }
...
impl NoApplIDsRepeatingGroup {
    ...
    pub fn new() -> Self {
        let template: GroupTemplate = vec![
            ...
            group_element(tag::APPL_NEW_SEQ_NUM),
