...
impl News {
    ...
    pub fn set_no_news_ref_i_ds(&mut self, v: isize) {
        self.message.body.set_field(tag::NO_NEWS_REF_I_DS, fixer::fix_int::FIXInt::from(v));
    }

    /// Gets `NoNewsRefIDs`, Tag 1475.
    pub fn get_no_news_ref_i_ds(&self) -> Result<isize, MessageRejectErrorEnum> {
        let mut fld = field::NoNewsRefIDsField::new(0);
        self.message.body.get_field(tag::NO_NEWS_REF_I_DS, &mut fld.0)?;
        Ok(fld.value())
    }
