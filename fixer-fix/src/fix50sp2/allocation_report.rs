...
impl AllocationReport {
    ...
    pub fn new(alloc_report_id: field::AllocReportIDField, alloc_trans_type: field::AllocTransTypeField, alloc_report_type: field::AllocReportTypeField, alloc_status: field::AllocStatusField, side: field::SideField, quantity: field::QuantityField, avg_px: field::AvgPxField, trade_date: field::TradeDateField) -> Self {
        let mut msg = Message::new();
        msg.header.set_field(tag::MSG_TYPE, FIXString::from("AS".to_string()));

        msg.body.set_field(tag::ALLOC_REPORT_ID, alloc_report_id.0);

        msg.body.set_field(tag::ALLOC_TRANS_TYPE, alloc_trans_type.0);

        msg.body.set_field(tag::ALLOC_REPORT_TYPE, alloc_report_type.0);
    ...
    pub fn set_alloc_report_ref_id(&mut self, v: String) {
        self.message.body.set_field(tag::ALLOC_REPORT_REF_ID, FIXString::from(v));
    }

    /// Gets `AllocReportRefID`, Tag 795.
    pub fn get_alloc_report_ref_id(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::AllocReportRefIDField::new(String::new());
        self.message.body.get_field(tag::ALLOC_REPORT_REF_ID, &mut fld.0)?;
        Ok(fld.value().to_string())
    }
    ...
    pub fn has_alloc_report_ref_id(&self) -> bool {
        self.message.body.has(tag::ALLOC_REPORT_REF_ID)
    }
...
/// `RouteOut` is the callback type for routing `AllocationReport` messages.
pub type RouteOut = fn(msg: AllocationReport, session_id: SessionID) -> Result<(), MessageRejectErrorEnum>;
...
/// Returns the begin string, message type, and route function for `AllocationReport`.
pub fn route(router: RouteOut) -> Route {
    let r = move |msg: &Message, session_id: SessionID| -> Result<(), MessageRejectErrorEnum> {
        router(AllocationReport::from_message(msg.clone()), session_id)
    };
    ("9", "AS", Box::new(r))
}
