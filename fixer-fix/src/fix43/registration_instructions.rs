...
impl<G: Borrow<Group>> NoRegistDtls<G> {
    ...
    /// Gets `RegistDetls`, Tag 509.
    pub fn get_regist_detls(&self) -> Result<String, MessageRejectErrorEnum> {
        let mut fld = field::RegistDetlsField::new(String::new());
        self.group().field_map.get_field(tag::REGIST_DETLS, &mut fld.0)?;
        Ok(fld.value().to_string())
    }


    /// Returns true if `RegistDetls` is present, Tag 509.
    pub fn has_regist_detls(&self) -> bool {
        self.group().field_map.has(tag::REGIST_DETLS)
    }
...
impl<G: BorrowMut<Group>> NoRegistDtls<G> {
    ...
    /// Sets `RegistDetls`, Tag 509.
    pub fn set_regist_detls(&mut self, v: String) {
        self.group_mut().field_map.set_field(tag::REGIST_DETLS, FIXString::from(v));
    }
...
impl NoRegistDtlsRepeatingGroup {
    ...
    pub fn new() -> Self {
        let template: GroupTemplate = vec![
            ...
            group_element(tag::REGIST_DETLS),
